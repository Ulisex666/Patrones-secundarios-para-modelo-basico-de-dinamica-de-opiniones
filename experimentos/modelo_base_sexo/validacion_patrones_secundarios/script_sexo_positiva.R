...
patron_sexo_positiva <- read.csv("data_csv/patron-edad-positiva-table.csv", skip = 6)
...
patron_sexo_positiva <- patron_sexo_positiva %>% 
  rename(
    tick = X.step., 
...
patron_voto_general <- patron_sexo_positiva %>% 
  group_by(tick) %>% 
...
patron_voto_hombres <- patron_sexo_positiva %>% 
  group_by(tick) %>% 
...
patron_voto_mujeres <- patron_sexo_positiva %>% 
  group_by(tick) %>% 
...
general_RMSE <- patron_sexo_positiva %>% 
...
hombre_RMSE <- patron_sexo_positiva %>% 
...
mujer_RMSE <- patron_sexo_positiva %>% 
...
patron_general_plot <- ggplot(patron_voto_general, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference evolution and standard deviation in positive influence model",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  "), c(pairs_per_tick, learning_rate), collapse = ", "),
    x = "Ticks (days)",
...
patron_voto_hombres_plot <- ggplot(patron_voto_hombres, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference share by men in positive influence",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  "), c(pairs_per_tick, learning_rate), collapse = ", "),
    x = "Ticks (days)",
...
patron_voto_mujeres_plot <- ggplot(patron_voto_mujeres, aes(x = tick)) +
  geom_ribbon(aes(ymin = mean_mujer_A - sd_mujer_A, ymax = mean_mujer_A + sd_mujer_A), 
              fill = "purple1", alpha = 0.2) +
  geom_line(aes(y = mean_mujer_A), color = "purple4", size = 1) +
  geom_point(data = encuestas_preferencias_mujeres[2:3,], 
             aes(x = tick, y = porcentaje_votos), 
  ...
  theme_minimal() +
  labs(
    title = "Preference share by women in positive influence",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  "), c(pairs_per_tick, learning_rate), collapse = ", "),
    x = "Ticks (days)",
    y = "Male preference share for A (%)"
  ) +
...
grafica_convergencia_positiva <- ggplot() +
  ...
  labs(
    title = "Preference evolution by gender under positive influence",
...
print(grafica_convergencia_positiva)
ggsave("figures/positive_influence_evolution_gender.png", plot = grafica_convergencia_positiva,
