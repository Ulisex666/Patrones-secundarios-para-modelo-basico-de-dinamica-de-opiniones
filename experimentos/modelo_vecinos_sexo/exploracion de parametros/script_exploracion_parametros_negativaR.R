...
vecinos_negativa_exploracion <- read.csv("datos_csv/exploracion-parametros-vecinos-negativa-table.csv", skip = 6)

# Se le cambian los nombres a los datos para manejarlos de forma mas sencilla
vecinos_negativa_exploracion <- vecinos_negativa_exploracion %>%
  rename(
    tick = X.step., 
  ...
  select(tick, run_number, pairs_per_tick, learning_rate, confidence_threshold, pref_A, percentage_A)
...
vecinos_negativa_best_configuration <- vecinos_negativa_exploracion %>% 
  filter(learning_rate == best_lr & pairs_per_tick == best_pairs & confidence_threshold == best_ct)
...
vecinos_negativa_error_by_run <- vecinos_negativa_exploracion %>% 
...
vecinos_negativa_error_mean <- vecinos_negativa_error_by_run%>% 
  # Se anegativan base a los parametros
  ...
  summarise(
    ...
    .groups = "drop"
  ) %>% # Se filtra CT=0, se sabe que esa no de buenos resultados
  filter(confidence_threshold != 0)

vecinos_negativa_top_best <- vecinos_negativa_error_mean %>% 
...
vecinos_negativa_error_plot <- ggplot(vecinos_negativa_error_mean, aes(x = pairs_per_tick, y = learning_rate, 
                                                                       fill = mean_RMSE)) + 
  geom_tile() +
  ...
  labs(
    title = "RMSE across 3 parameters for negative influence under neighbor interactions",
    x = "Learning rate",
    y = "Agents per tick",
    fill = "RMSE (%)"
  ) +
...
ggsave("figures/vecinos_negativa_error.png", plot = vecinos_negativa_error_plot,
...
vecinos_negativa_lollipop_plot <- ggplot(vecinos_negativa_top_best, aes(x = etiqueta, y = mean_RMSE)) +
  ...
  coord_flip() +
  labs(
    title = "Top 5 configurations for negative influence
    under neighbor interaction",
    subtitle = "Ranked by lowest mean RMSE | Error bars show Standard Deviation (30 runs)",
    x = "Parameters (Pairs per tick | Learning Rate | Confidence threshold)",
    y = "Mean RMSE (%)"
  ) +
  theme_minimal() +
  theme(
...
print(vecinos_negativa_lollipop_plot)
ggsave("figures/vecinos_negativa_lollipot.png", plot = vecinos_negativa_lollipop_plot,
       width = 8, height = 5, dpi = 300)
...
best_RMSE = vecinos_negativa_top_best[1, ]$mean_RMSE
vecinos_negativa_best_configuration <- read.csv(file = "datos_csv/vecinos_negativa_best_config.csv")

vecinos_negativa_mejor_config_avg <-  vecinos_negativa_best_configuration %>%  
  group_by(tick) %>% 
...
vecinos_negativa_best_distance <- encuestas_reales %>%
  inner_join(vecinos_negativa_mejor_config_avg, by = "tick") %>%
  mutate(
...
vecinos_negativa_best_plot <- ggplot(vecinos_negativa_mejor_config_avg, aes(x = tick)) +
  ...
  geom_segment(data = vecinos_negativa_best_distance,
               aes(x = tick, xend = tick, y = pct_A_real, yend = mean_pct_A),
               linetype = "dashed", color = "grey40", linewidth = 0.5) +
  geom_text(data = vecinos_negativa_best_distance,
  ...
  theme_minimal() +
  labs(
    title = "Preference evolution for best configuration in negative influence,
    neighbor interactions",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  ", "confidence threshold = ","Mean RMSE = "), 
                      c(best_pairs, best_lr, best_ct, round(best_RMSE, 2)), collapse = ", "),
    x = "Ticks (days)",
    y = "Preference share for A (%)"
  ) +
  scale_y_continuous(limits = c(60,70), breaks = c(60:70))

print(vecinos_negativa_best_plot)
ggsave("figures/vecinos_negativa_best.png", plot = vecinos_negativa_best_plot,
       width = 8, height = 5, dpi = 300)
