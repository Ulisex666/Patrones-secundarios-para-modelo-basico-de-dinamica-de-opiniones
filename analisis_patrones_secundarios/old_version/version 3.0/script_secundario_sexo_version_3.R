...
setwd("C:/Users/ulise/Documents/GitHub/Modelo-base-dinamica-de-opinion/experimentos/patrones secundarios/version 3.0/datos_experimentos")
library(tidyverse)

experimento_secundario_sexo <- read.csv('experimento-sexo-ajustado.csv', skip = 6)
...
ggplot(preferencias_A_mujeres_pct, aes(x = category, y = promedio_pct)) +
  ...
  annotate("text", x = 1, y = 72, label = "Simulated value (mean = 68 %)", 
  ...
  theme_minimal() +
  scale_y_continuous(limits = c(0, 72), breaks = seq(0, 80, 10)) +
  theme(
...
ggplot(preferencias_A_hombres_pct, aes(x = category, y = promedio_pct)) +
  ...
  annotate("text", x = 1, y = 65, label = "Simulated value (mean = 65 %)", 
           color = "grey", fontface = "bold") +
