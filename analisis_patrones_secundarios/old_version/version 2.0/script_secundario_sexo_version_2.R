...
setwd("C:/Users/ulise/Documents/GitHub/Modelo-base-dinamica-de-opinion/experimentos/patrones secundarios/version 2.0/datos_experimentos")
library(tidyverse)

experimento_secundario_sexo <- read.csv('experimento-secundario-sexo.csv', skip = 6)
...
ggplot(preferencias_A_mujeres_pct, aes(x = category, y = promedio_pct)) +
  ...
  annotate("text", x = 1, y = 66, label = "Simulated value (mean = 66 %)", 
...
ggplot(preferencias_A_hombres_pct, aes(x = category, y = promedio_pct)) +
  ...
  annotate("text", x = 1, y = 66, label = "Simulated value (mean = 67 %)", 
           color = "grey", fontface = "bold") +
