...
patron_sexo_bc <- read.csv("data_csv/patron-edad-BC-table.csv", skip = 6)
...
patron_voto_general <- patron_sexo_bc %>% 
  group_by(tick) %>% 
...
patron_voto_hombres <- patron_sexo_bc %>% 
  group_by(tick) %>% 
...
patron_voto_mujeres <- patron_sexo_bc %>% 
  group_by(tick) %>% 
...
patron_general_plot <- ggplot(patron_voto_general, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference evolution and standard deviation in
...
patron_voto_hombres_plot <- ggplot(patron_voto_hombres, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference share by men in bounded confidence",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  ", "confidence threshold = "),
                      c(pairs_per_tick, learning_rate, confidence_threshold), collapse = ", "),
    x = "Ticks (days)",
...
patron_voto_mujeres_plot <- ggplot(patron_voto_mujeres, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference share by women in bounded confidence",
    subtitle = paste0(c("Pairs per tick = ", "learning rate =  ", "confidence threshold = "), 
                      c(pairs_per_tick, learning_rate, confidence_threshold), collapse = ", "),
    x = "Ticks (days)",
    y = "Male preference share for A (%)"
...
grafica_convergencia_bc <- ggplot() +
  ...
  labs(
    title = "Preference evolution by gender under bounded confidence",
...
print(grafica_convergencia_bc)
ggsave("figures/bc_influence_evolution_gender.png", plot = grafica_convergencia_bc,
